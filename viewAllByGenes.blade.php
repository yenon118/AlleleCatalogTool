                ...
                if ($key != "Gene" && $key != "Chromosome" && $key != "Position" && $key != "Genotype" && $key != "Genotype_Description") {
                    ...
                    for ($j = 0; $j < count($position_array); $j++) {
                        if ($organism == "Osativa" && $dataset == "Rice3000") {
                            echo "<th style=\"border:1px solid black; min-width:80px;\"><a href=\"../viewVariantAndPhenotype/" . $organism . "?Dataset=" . $dataset . "&Chromosome=" . $allele_catalog_result_arr[$i][0]->Chromosome . "&Position=" . $position_array[$j] . "&Gene=" . $allele_catalog_result_arr[$i][0]->Gene . "\" target=\"_blank\">" . $position_array[$j] . "</a></th>";
